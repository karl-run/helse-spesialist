...
internal class ApiVarselDaoTest: DatabaseIntegrationTest() {
    ...
    fun `Finner varsler som skal med for siste snapshotgenerasjon`() {
        ...
        val utbetalingId = UUID.randomUUID()
        opprettVedtaksperiode(opprettPerson(), opprettArbeidsgiver())
        opprettVarseldefinisjon(kode = "KODE_1")
        val defId2 = UUID.randomUUID()
        val generasjonId2 = UUID.randomUUID()
        opprettVarseldefinisjon(kode = "KODE_2", definisjonId = defId2)
        opprettVarseldefinisjon(kode = "KODE_3")
        opprettVarseldefinisjon(kode = "KODE_4")
        val generasjonRef1 = nyGenerasjon(vedtaksperiodeId = vedtaksperiodeId)
        val generasjonRef2 = nyGenerasjon(vedtaksperiodeId = vedtaksperiodeId, utbetalingId = utbetalingId, generasjonId = generasjonId2)
        val generasjonRef3 = nyGenerasjon(vedtaksperiodeId = vedtaksperiodeId)
        nyttVarsel(kode = "KODE_1", vedtaksperiodeId = vedtaksperiodeId, generasjonRef = generasjonRef1)
        nyttVarsel(kode = "KODE_2", vedtaksperiodeId = vedtaksperiodeId, generasjonRef = generasjonRef2)
        nyttVarsel(kode = "KODE_3", vedtaksperiodeId = vedtaksperiodeId, generasjonRef = generasjonRef2)
        nyttVarsel(kode = "KODE_4", vedtaksperiodeId = vedtaksperiodeId, generasjonRef = generasjonRef3)
        val varsler = apiVarselDao.finnVarslerSomIkkeErInaktiveForSisteGenerasjon(vedtaksperiodeId, utbetalingId)

        val expectedVarsel = Varsel(generasjonId2, defId2, "KODE_2", "EN_TITTEL", null, null, null)
        assertTrue(varsler.contains(expectedVarsel))
        assertTrue(varsler.isNotEmpty())
        assertEquals(setOf("KODE_2", "KODE_3", "KODE_4",), varsler.map { it.toDto().kode }.toSet())
    }
